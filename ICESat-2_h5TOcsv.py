...
def h5TOcsv(h5file, beam, bound=[111.59, 16.535, 111.62, 16.55]):
    ...
    coorSet = np.vstack((lon_ph, lat_ph, h_ph, dist_ph_along))
...
def h5TOcsv(h5file, csvfile, beam):
    '''beam为激光波束，在ICESat-2中分别有gt1l,gt1r,gt2l,gt2r,gt3l,gt3r六种激光波束'''
    # HDF5的读取：
    print("Running:Open h5file")
    f = h5py.File(h5file, 'r')  # 打开h5文件
    ...
    dist_ph_across = f[beam + '/heights/dist_ph_across'][:]
    dist_ph_along = f[beam + '/heights/dist_ph_along'][:]
    # pce_mframe_cnt=f['gt1l/heights/pce_mframe_cnt'][:]
    # ph_id_channel=f['gt1l/heights/ph_id_channel'][:]
    # ph_id_count=f['gt1l/heights/ph_id_count'][:]
    # ph_id_pulse=f['gt1l/heights/ph_id_pulse'][:]
    # signal_conf_ph = f['gt1l/heights/signal_conf_ph'][:]
    f.close()
    ...
    invalidDataSet = []
    for i in range(0, len(coorSet_T)):
        # if (coorSet_T[i, 0] > 113) or (coorSet_T[i, 0] < 111):
        #     invalidDataSet.append(i)
        #     continue

        if (coorSet_T[i, 1] > 16.55) or (coorSet_T[i, 1] < 16.535):
            invalidDataSet.append(i)
            continue
